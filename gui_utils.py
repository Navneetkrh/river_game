...
class GuiUtils:
    ...
    def render_game_over_screen(self):
        """Render the game over screen with restart and main menu buttons"""
        if self.begin_centered_window("Game Over", 300, 200):
            self.draw_text_centered("Game Over!")
            self.add_spacing(20)
            if self.draw_centered_button("Restart", 200, 40):
                return "restart"
            self.add_spacing(10)
            if self.draw_centered_button("Main Menu", 200, 40):
                return "main_menu"
            imgui.end()
        return None

    def render_win_screen(self):
        """Render the win screen with restart and main menu buttons"""
        if self.begin_centered_window("You Win!", 300, 200):
            self.draw_text_centered("You Win!")
            self.add_spacing(20)
            if self.draw_centered_button("Next Level", 200, 40):
                return "next_level"
            self.add_spacing(10)
            if self.draw_centered_button("Main Menu", 200, 40):
                return "main_menu"
